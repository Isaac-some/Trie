...
type PathPreview = { source: string; destination: string; sourcePrefix: string; destinationPrefix: string };
...
function DatasetWorkspace({ dataset, destinations, onMappingsChange }: { dataset: Dataset; destinations: Destination[]; onMappingsChange: (datasetId: string, mappings: Mapping[]) => void }) {
  ...
  return <section className="mapping-workspace"><header className="mapping-header"><div><h4>起止点生成器</h4><p>从这棵路径树选择起点，再从上方终点库完成匹配。</p></div><span>{sources.length} 个起点 / {pairedGroups.length} 组传送对</span></header><div className="mapping-columns"><section className="source-area"><div className="mapping-section-title"><div><h5>起点分支</h5><p>从路径树中挑选要传送的目录。</p></div><button className="secondary-button" onClick={() => setPickerOpen(true)}>选择分支</button></div>{sources.length ? <div className="source-list">{sources.map((source, index) => <div className={destinationById.get(assignments[source]) ? 'source-row is-matched' : 'source-row'} key={source}><b>{String(index + 1).padStart(2, '0')}</b><div><code title={source}>{shortPath(source)}</code><label>传送到<select value={assignments[source] || ''} onChange={(event) => setAssignments((current) => ({ ...current, [source]: event.target.value }))}><option value="">选择终点</option>{destinations.map((destination) => <option key={destination.id} value={destination.id} disabled={!destination.path.trim()}>{destination.path.trim() || '未填写的终点'}</option>)}</select></label></div><button className="remove-button" onClick={() => updateSources(sources.filter((item) => item !== source))}>移除</button></div>)}</div> : <div className="quiet large">还没有起点分支。点击“选择分支”开始。</div>}</section><section className="route-preview-area" aria-label="已匹配传送对"><div className="mapping-section-title"><div><h5>已匹配传送对</h5><p>完成选择后会在这里生成起点到终点的关系。</p></div>{pairedGroups.length > 0 && <span className="pair-count">{pairedGroups.length} 组</span>}</div>{pairedGroups.length ? <PairGroups datasetId={dataset.id} groups={pairedGroups} /> : <div className="quiet large">选择起点并指定终点后，这里会显示传送对。</div>}</section></div><footer className="mapping-footer"><div><strong>{allMatched ? '本树的传送对已汇入顶部的统一导出。' : '每个起点都需要选择一个已填写的终点。'}</strong></div></footer><BranchPickerDialog open={pickerOpen} datasetId={dataset.id} selected={sources} onCancel={() => setPickerOpen(false)} onConfirm={(nextSources) => { updateSources(nextSources); setPickerOpen(false); }} /></section>;
}

function PairGroups({ datasetId, groups }: { datasetId: string; groups: Array<{ destination: Destination; sources: string[] }> }) {
  const [previews, setPreviews] = useState<PathPreview[] | null>(null);
  const [previewDestination, setPreviewDestination] = useState('');
  const [previewError, setPreviewError] = useState('');
  const [previewing, setPreviewing] = useState<string | null>(null);
  const openPreview = async (group: { destination: Destination; sources: string[] }) => {
    const destination = group.destination.path.trim();
    setPreviewing(group.destination.id);
    setPreviewError('');
    try {
      const result = await api<{ previews: PathPreview[] }>(`/api/datasets/${datasetId}/previews`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mappings: group.sources.map((source) => ({ source, destination })) }) });
      setPreviewDestination(destination);
      setPreviews(result.previews);
    } catch (error) { setPreviewError(error instanceof Error ? error.message : '无法读取传输预览。'); }
    finally { setPreviewing(null); }
  };
  return <><div className="pair-list">{groups.map((group, index) => <article className="pair-group" key={group.destination.id}><header><div><strong>传送对 {String(index + 1).padStart(2, '0')}</strong><span>{group.sources.length} 个起点</span></div><button className="preview-button" disabled={previewing === group.destination.id} onClick={() => void openPreview(group)}>{previewing === group.destination.id ? '正在生成预览' : '查看前缀替换预览'}</button></header><div className="pair-route"><div className="pair-endpoint"><span>起点</span><div className="pair-source-chips">{group.sources.map((source) => <code key={source} title={source}>{shortPath(source)}</code>)}</div></div><div className="pair-arrow" aria-hidden="true">→</div><div className="pair-endpoint pair-destination"><span>终点</span><code title={group.destination.path.trim()}>{group.destination.path.trim()}</code></div></div></article>)}</div>{previewError && <p className="inline-error">{previewError}</p>}{previews !== null && <PrefixPreviewDialog destination={previewDestination} previews={previews} onClose={() => setPreviews(null)} />}</>;
}

function PrefixPreviewDialog({ destination, previews, onClose }: { destination: string; previews: PathPreview[]; onClose: () => void }) {
  return <div className="preview-dialog-backdrop" role="presentation"><section className="preview-dialog" role="dialog" aria-modal="true" aria-labelledby="prefix-preview-title"><header><div><p className="eyebrow">传输预览</p><h4 id="prefix-preview-title">前缀替换结果</h4><p>所选分支会完整保留，并传送到 <code>{destination}</code> 下。</p></div><button className="remove-button" onClick={onClose}>关闭</button></header><div className="preview-paths">{previews.length ? <><div className="preview-legend" aria-label="颜色说明"><span className="preview-replaced">被替换</span><span className="preview-preserved">完整保留</span><span className="preview-inserted">替换为</span></div>{previews.map((preview) => <article key={`${preview.source}\n${preview.destination}`}><code title={preview.source}><mark className="preview-replaced">{preview.sourcePrefix}</mark><span className="preview-preserved">{preview.source.slice(preview.sourcePrefix.length)}</span></code><span aria-hidden="true">→</span><code title={preview.destination}><mark className="preview-inserted">{preview.destinationPrefix}</mark><span className="preview-preserved">{preview.destination.slice(preview.destinationPrefix.length)}</span></code></article>)}</> : <p className="quiet large">当前选择下未找到可预览的路径。</p>}</div><footer><span>以上为源 CSV 中的真实路径样本；导出会使用同一替换规则。</span><button className="primary" onClick={onClose}>知道了</button></footer></section></div>;
}
